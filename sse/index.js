...
app.get("/", (req, res) => res.send("hey!show me some roman numerals"));

/**
 * to unlock the SSE at the backend, we need a special get request
 */
...
const serverName = process.env.SERVER_NAME || "RomanNumeralServer";
let num = 0
let numing = 0
...
function send (res) {
    ...
    // res.write(`data: ${romanNumerals(num++)}\n\n`);
    res.write(`data: for ${++num}, we have ${romanNumerals(num)} and for ${++num} we have ${romanNumerals(num)}\n\n`);
